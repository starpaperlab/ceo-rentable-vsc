...
export async function handleWorkspaceInvitation(payload = {}, { env = process.env, headers = {} } = {}) {
  ...
  if (profile?.id) {
    const { error: membershipError } = await supabase.from('workspace_members').upsert({ workspace_id: workspaceId, user_id: profile.id, role, status: 'active', module_permissions: modulePermissions }, { onConflict: 'workspace_id,user_id' });
