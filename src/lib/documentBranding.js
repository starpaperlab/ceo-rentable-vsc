...
function normalizeBusinessConfigSource(config = {}) {
  ...
  return {
    ...
    bank_account_type: pickString(config.bank_account_type),
    signature_name: pickString(config.signature_name),
    signature_title: pickString(config.signature_title),
    terms_text: pickString(config.terms_text),
    ...documentPreferences,
  };
}
...
export function resolveDocumentBranding(doc = {}, ownerConfig = null, defaults = DEFAULT_BRANDING) {
  ...
  return {
    ...
    bank_account_type: pickResolvedString(snapshotSource.bank_account_type, docSource.bank_account_type, ownerSource.bank_account_type, defaults.bank_account_type),
    signature_name: pickResolvedString(snapshotSource.signature_name, docSource.signature_name, ownerSource.signature_name, defaults.signature_name),
    signature_title: pickResolvedString(snapshotSource.signature_title, docSource.signature_title, ownerSource.signature_title, defaults.signature_title),
    terms_text: pickResolvedString(snapshotSource.terms_text, docSource.terms_text, ownerSource.terms_text, defaults.terms_text),
...
export function buildDocumentBrandingSnapshot(source = {}, { brandProfileId = null } = {}) {
  ...
  return {
    ...
    bank_account_type: resolved.bank_account_type || '',
    signature_name: resolved.signature_name || '',
    signature_title: resolved.signature_title || '',
    terms_text: resolved.terms_text || '',
    social_links: socialLinks,
    document_preferences: documentPreferences,
    pdf_preferences: documentPreferences,
    logo_settings: logoSettings,
...
export function buildDocumentBrandingFields(source = {}, { brandProfileId = null } = {}) {
  ...
  return {
    ...
    bank_account_type: snapshot.bank_account_type,
    signature_name: snapshot.signature_name,
    signature_title: snapshot.signature_title,
...
export function mapBrandProfileToBusinessConfig(profile = {}, { ownerName = '', ownerEmail = '' } = {}) {
  ...
  return {
    ...
    bank_account_type: normalized.bank_account_type || '',
    signature_name: normalized.signature_name || '',
    signature_title: normalized.signature_title || '',
