  ...
  return <div className={`grid grid-cols-2 ${compact ? 'gap-2.5' : 'gap-3'}`} role="radiogroup" aria-label="Frecuencia de facturación">
    {Object.values(CHECKOUT_PLANS).map((option) => {
      ...
      return <button key={option.code} type="button" role="radio" aria-checked={isSelected} onClick={() => onSelect(option.code)} disabled={disabled} className={`relative rounded-2xl border-2 text-left transition focus:outline-none focus:ring-2 focus:ring-[#D45387]/30 disabled:opacity-60 ${compact ? 'min-h-[108px] p-3.5' : 'min-h-[126px] p-4'} ${isSelected ? 'border-[#D45387] bg-[#D45387]/5 shadow-sm' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
        ...
        <p className={`${compact ? 'mt-1.5' : 'mt-2'} text-xs leading-4 text-gray-500`}>{isAnnual ? 'Equivale a US$17.50/mes' : 'Facturación cada mes'}</p>
      </button>;
    })}
  </div>;
}
...
export default function Checkout() {
  ...
  const handleCreateAccount = async (event) => {
    ...
    try {
      ...
      setAccountReady(true); setTermsAccepted(true); setMessage('Cuenta creada. Continúa con PayPal para iniciar tus 7 días gratis.');
  ...
  const handleSubscriptionApproved = async ({ subscriptionId, planCode }) => {
    ...
    try {
      ...
      setMessage(result.status === 'trialing' ? '¡Listo! Tu prueba de 7 días está activa y hoy pagaste US$0.' : '¡Listo! PayPal confirmó tu suscripción.');
  ...
  return <main className="min-h-screen bg-[#F7F3EE] px-3.5 py-5 sm:px-6 sm:py-8 lg:py-12"><div className="mx-auto max-w-6xl">
    ...
    <div className="grid overflow-hidden rounded-[26px] border border-gray-200 bg-white shadow-xl sm:rounded-3xl lg:grid-cols-[1.15fr_0.85fr]">
      <section className="p-5 sm:p-9 lg:p-12">
        <div className="mb-5 sm:mb-8"><p className="mb-1.5 text-xs font-bold uppercase tracking-wide text-[#D45387] sm:mb-2 sm:text-sm">{user ? 'Cuenta lista' : 'Crea tu cuenta'}</p><h1 className="text-[28px] font-black leading-[1.05] text-gray-900 sm:text-3xl">Empieza tus 7 días gratis</h1><p className="mt-2 text-sm leading-5 text-gray-600 sm:leading-6">{user ? 'Revisa tu plan y continúa con PayPal. Hoy pagas US$0.' : 'Completa tus datos una sola vez. Después continuarás directamente con PayPal. Hoy pagas US$0.'}</p></div>
        <div className="mb-5 rounded-2xl border border-[#F0D4DF] bg-[#FFF8FB] p-3.5 lg:hidden"><div className="mb-3 flex items-end justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-wide text-[#D45387]">Tu plan</p><p className="mt-0.5 text-sm font-black text-gray-900">Elige mensual o anual</p></div><p className="shrink-0 text-[11px] font-semibold text-gray-500">7 días gratis</p></div><PlanSelector plan={plan} onSelect={selectPlan} disabled={planSelectionDisabled} compact /></div>
        {user && <div className="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm leading-5 text-emerald-800"><CheckCircle2 className="mr-2 inline h-4 w-4" /><strong>Cuenta verificada.</strong> Continúa con PayPal para activar la prueba.</div>}
