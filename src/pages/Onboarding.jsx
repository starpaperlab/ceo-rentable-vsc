...
import { calculateLaborHourlyCost, calculateProductiveHours } from '@/lib/costEngine';
...
const emptyConfig = {
  ...
  monthly_capacity: '', monthly_capacity_unknown: false, operation_mode: '', labor_mode: '', team_size: '', personal_income_goal: '', productive_time_pct: 70,
...
export default function Onboarding() {
  ...
  const productiveHours = useMemo(()=>calculateProductiveHours({workDaysPerWeek:Number(config.work_days_per_week||0),workHoursPerDay:Number(config.work_hours_per_day||0),productivePct:Number(config.productive_time_pct||70)}),[config.work_days_per_week,config.work_hours_per_day,config.productive_time_pct]);
  const hourlyReference = useMemo(()=>calculateLaborHourlyCost({personalIncomeGoal:Number(config.personal_income_goal||0),workDaysPerWeek:Number(config.work_days_per_week||0),workHoursPerDay:Number(config.work_hours_per_day||0),productivePct:Number(config.productive_time_pct||70)}),[config.personal_income_goal,config.work_days_per_week,config.work_hours_per_day,config.productive_time_pct]);
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(async()=>{
      const payload={
        ...
        operation_mode: config.operation_mode || null,
        labor_mode: config.labor_mode || null,
        team_size: config.labor_mode === 'solo' ? 1 : (config.team_size === '' ? null : Number(config.team_size)),
        personal_income_goal: config.personal_income_goal === '' ? null : Number(config.personal_income_goal),
        productive_time_pct: Number(config.productive_time_pct || 70),
  ...
  const canContinue=()=>{
    ...
    if(step===3)return config.work_days_per_week!=='' && config.work_hours_per_day!=='' && (config.monthly_capacity_unknown || config.monthly_capacity!=='') && Boolean(config.operation_mode) && Boolean(config.labor_mode) && (config.labor_mode==='solo' || config.team_size!=='');
    ...
    <div className="min-h-[100dvh] bg-background text-foreground">
      <div className="mx-auto w-full max-w-3xl px-4 py-5 sm:px-6 sm:py-8">
        ...
        <div className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7">
          ...
          {step===3 && <div className="space-y-5"><div><h2 className="text-2xl font-bold">Cuéntanos tu capacidad normal</h2><p className="mt-2 text-sm text-muted-foreground">No buscamos exactitud contable. Una aproximación útil es suficiente.</p></div><div className="grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-semibold">Días por semana<Input className={inputClass} type="number" min="0" max="7" value={config.work_days_per_week??''} onChange={e=>setConfig(p=>({...p,work_days_per_week:e.target.value}))}/></label><label className="space-y-2 text-sm font-semibold">Horas al día<Input className={inputClass} type="number" min="0" max="24" step="0.5" value={config.work_hours_per_day??''} onChange={e=>setConfig(p=>({...p,work_hours_per_day:e.target.value}))}/></label></div><label className="block space-y-2 text-sm font-semibold">{template.capacityLabel}<Input className={inputClass} type="number" min="0" disabled={config.monthly_capacity_unknown} value={config.monthly_capacity??''} onChange={e=>setConfig(p=>({...p,monthly_capacity:e.target.value,monthly_capacity_unknown:false}))}/></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={config.monthly_capacity_unknown} onChange={e=>setConfig(p=>({...p,monthly_capacity_unknown:e.target.checked,monthly_capacity:e.target.checked?'':p.monthly_capacity}))}/> No estoy segura</label><div className="space-y-3 pt-2"><p className="text-sm font-semibold">¿Cómo realizas normalmente el trabajo?</p><div className="grid gap-2 sm:grid-cols-2">{(OPERATION_OPTIONS[config.business_model]||OPERATION_OPTIONS.products).map(([id,label])=><ToggleCard key={id} selected={config.operation_mode===id} title={label} onClick={()=>setConfig(p=>({...p,operation_mode:id}))}/>)}</div></div><div className="space-y-3 pt-2"><p className="text-sm font-semibold">¿Quién realiza el trabajo principal?</p><div className="grid gap-2 sm:grid-cols-2">{LABOR_OPTIONS.map(([id,label])=><ToggleCard key={id} selected={config.labor_mode===id} title={label} onClick={()=>setConfig(p=>({...p,labor_mode:id,team_size:id==='solo'?1:p.team_size}))}/>)}</div>{config.labor_mode&&config.labor_mode!=='solo'&&<label className="block space-y-2 text-sm font-semibold">¿Cuántas personas participan aproximadamente?<Input className={inputClass} type="number" min="1" value={config.team_size??''} onChange={e=>setConfig(p=>({...p,team_size:e.target.value}))}/></label>}</div></div>}

          {step===4 && <div className="space-y-5"><div><h2 className="text-2xl font-bold">¿Cuánto te gustaría ganar personalmente al mes?</h2><p className="mt-2 text-sm text-muted-foreground">Es el dinero que quieres recibir tú. No son las ventas totales del negocio.</p></div><div className="max-w-sm"><label className="text-sm font-semibold">Meta mensual personal</label><div className="relative mt-2"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">RD$</span><Input className={`${inputClass} pl-12`} inputMode="decimal" type="number" min="0" value={config.personal_income_goal??''} onChange={e=>setConfig(p=>({...p,personal_income_goal:e.target.value}))}/></div></div><div className="rounded-2xl border border-border bg-background p-4"><p className="font-semibold">No todas tus horas son horas de producción</p><p className="mt-1 text-xs text-muted-foreground">Mensajes, compras, administración y seguimiento también consumen tiempo. CEO Rentable recomienda empezar con 70% de tiempo productivo y tú puedes cambiarlo.</p><div className="mt-3 grid grid-cols-3 gap-2">{[60,70,80].map(pct=><button key={pct} type="button" onClick={()=>setConfig(p=>({...p,productive_time_pct:pct}))} className={`rounded-xl border px-3 py-2 text-sm font-semibold ${Number(config.productive_time_pct)===pct?'border-primary bg-primary/10 text-primary':'border-border'}`}>{pct}%</button>)}</div><label className="mt-3 block space-y-2 text-sm font-semibold">Tiempo productivo estimado %<Input className={inputClass} inputMode="decimal" type="number" min="1" max="100" step="1" value={config.productive_time_pct??70} onChange={e=>setConfig(p=>({...p,productive_time_pct:e.target.value}))}/></label></div>{Number(config.personal_income_goal||0)>0&&productiveHours>0&&<div className="rounded-2xl border border-primary/25 bg-primary/5 p-4"><p className="text-sm text-muted-foreground">Tu hora de trabajo estimada</p><p className="mt-1 text-2xl font-black text-primary">{money(hourlyReference)} / hora</p><p className="mt-2 text-xs text-muted-foreground">Meta {money(config.personal_income_goal)} ÷ {productiveHours.toFixed(1)} horas productivas aproximadas al mes. Puedes ajustar el porcentaje anterior y ver cómo cambia.</p></div>}</div>}

          {step===5 && <ExpenseStep mode="expenses" suggestions={template.expenses} rows={expenses} setRows={setExpenses} workspaceId={activeWorkspaceId} user={user} onError={setError} businessConfig={config}/>} 

          {step===6 && <ExpenseStep mode="subscriptions" suggestions={template.subscriptions} rows={expenses} setRows={setExpenses} workspaceId={activeWorkspaceId} user={user} onError={setError} businessConfig={config}/>} 

          {step===7 && <MaterialStep title={productBusiness?'Materiales e insumos':'Materiales (opcional)'} subtitle={productBusiness?'Dinos cuánto pagaste y cuánta cantidad compraste. CEO Rentable calcula automáticamente el costo unitario y el consumo.':'Si tu servicio utiliza materiales, puedes registrarlos ahora o completar después.'} suggestions={template.materials} rows={materials} setRows={setMaterials} workspaceId={activeWorkspaceId} user={user} onError={setError}/>} 
