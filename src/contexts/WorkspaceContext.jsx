...
export function WorkspaceProvider({ children }) {
  const { user, userProfile } = useAuth(); const [workspaces,setWorkspaces]=useState([]); const [activeWorkspaceId,setActiveWorkspaceIdState]=useState(null); const [isLoadingWorkspace,setIsLoadingWorkspace]=useState(false); const [workspaceError,setWorkspaceError]=useState(null);
  ...
  const loadWorkspaces=useCallback(async()=>{ if(!user?.id){setWorkspaces([]);setActiveWorkspaceIdState(null);setWorkspaceError(null);return;} setIsLoadingWorkspace(true);setWorkspaceError(null);
    try { let {data:memberships,error:membershipsError}=await supabase.from('workspace_members').select('workspace_id,role,status,module_permissions,created_at').eq('user_id',user.id).eq('status','active').order('created_at',{ascending:true});
      ...
      const {data:workspaceRows,error:workspacesError}=await supabase.from('workspaces').select('id,name,slug,owner_user_id,country_code,currency_code,timezone,logo_url,brand_primary_color,brand_accent_color,team_enabled,seat_limit,settings,created_at,updated_at').in('id',ids); if(workspacesError)throw workspacesError;
      ...
      const stored=storageKey?window.localStorage.getItem(storageKey):null; const paidOwnerAccess=userProfile?.role==='admin'||(userProfile?.has_access===true&&['monthly','founder','founder_lifetime','subscription'].includes(userProfile?.plan)&&(userProfile?.access_status??'active')==='active'); const storedWorkspace=normalized.find(w=>w.id===stored); const storedAllowed=Boolean(storedWorkspace&&(paidOwnerAccess||storedWorkspace.role!=='owner')); const defaultWorkspace=paidOwnerAccess?normalized[0]:normalized.find(w=>w.role!=='owner')||normalized[0]; const nextId=storedAllowed?stored:defaultWorkspace?.id||null; setActiveWorkspaceIdState(nextId); if(storageKey&&nextId)window.localStorage.setItem(storageKey,nextId);
    } catch(error){console.error('Workspace initialization failed',error);setWorkspaceError(error);if(isWorkspaceSchemaMissing(error)){setWorkspaces([legacyWorkspaceFor(user)]);setActiveWorkspaceIdState(null);}else{setWorkspaces([]);setActiveWorkspaceIdState(null);}} finally{setIsLoadingWorkspace(false);} },[storageKey,user,userProfile?.access_status,userProfile?.has_access,userProfile?.plan,userProfile?.role]);
