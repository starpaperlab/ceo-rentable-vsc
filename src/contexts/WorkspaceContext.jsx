...
export function WorkspaceProvider({ children }) {
  const { user } = useAuth(); const [workspaces,setWorkspaces]=useState([]); const [activeWorkspaceId,setActiveWorkspaceIdState]=useState(null); const [isLoadingWorkspace,setIsLoadingWorkspace]=useState(false); const [workspaceError,setWorkspaceError]=useState(null);
  ...
  const loadWorkspaces=useCallback(async()=>{ if(!user?.id){setWorkspaces([]);setActiveWorkspaceIdState(null);setWorkspaceError(null);return;} setIsLoadingWorkspace(true);setWorkspaceError(null);
    try { let {data:memberships,error:membershipsError}=await supabase.from('workspace_members').select('workspace_id,role,status,module_permissions,created_at').eq('user_id',user.id).eq('status','active').order('created_at',{ascending:true});
      ...
      const stored=storageKey?window.localStorage.getItem(storageKey):null; const nextId=normalized.some(w=>w.id===stored)?stored:normalized[0]?.id||null; setActiveWorkspaceIdState(nextId); if(storageKey&&nextId)window.localStorage.setItem(storageKey,nextId);
    } catch(error){console.error('Workspace initialization failed',error);setWorkspaceError(error);setWorkspaces([legacyWorkspaceFor(user)]);setActiveWorkspaceIdState(null);} finally{setIsLoadingWorkspace(false);} },[storageKey,user]);
